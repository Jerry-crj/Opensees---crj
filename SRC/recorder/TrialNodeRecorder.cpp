...
// Written: fmk 
//
// Description: This file contains the class definition for TrialNodeRecorder.
// A TrialNodeRecorder is used to record the specified dof responses 
// at a collection of nodes over an analysis. (between commitTag of 0 and
// last commitTag).
//
// What: "@(#) TrialNodeRecorder.C, revA"

#include <TrialNodeRecorder.h>
#include <Domain.h>
#include <Node.h>
#include <NodeIter.h>
#include <Vector.h>
...
OPS_TrialNodeRecorder()
{
    ...
    if (domain == 0)
        return 0;
    TrialNodeRecorder* recorder = new TrialNodeRecorder(dofs, &nodes, sensitivity,
...
TrialNodeRecorder::TrialNodeRecorder()
:Recorder(RECORDER_TAGS_TrialNodeRecorder),
...
TrialNodeRecorder::TrialNodeRecorder(const ID &dofs, 
			   const ID *nodes, 
			   int psensitivity,
 ...
 theTimeSeries(theSeries), timeSeriesValues(0)
{
  ...
  if (numDOF != 0) {
	...
	opserr << "TrialNodeRecorder::TrialNodeRecorder - invalid dof  " << dof;
  ...
  if (nodes != 0) {
	...
	opserr << "TrialNodeRecorder::TrialNodeRecorder - out of memory\n";
      }
    }
  } 
  ...
  if (dataToStore == 0 || (strcmp(dataToStore, "disp") == 0)) {
    ...
    dataFlag = 10;
    opserr << "TrialNodeRecorder::TrialNodeRecorder - dataToStore " << dataToStore;
...
int 
TrialNodeRecorder::record(int commitTag, double timeStamp)
{
  ...
  if (theOutputHandler == 0) {
    opserr << "TrialNodeRecorder::record() - no DataOutputHandler has been set\n";
    return -1;
  }
  
  if (initializationDone == false) {
    if (this->initialize() != 0) {
      opserr << "TrialNodeRecorder::record() - failed in initialize()\n";
      return -1;
    }
  }
...
int 
TrialNodeRecorder::setDomain(Domain &theDom)
{
  theDomain = &theDom;
  return 0;
}
...
int 
TrialNodeRecorder::sendSelf(int commitTag, Channel &theChannel)
{
  addColumnInfo = 1;

  if (theChannel.isDatastore() == 1) {
    opserr << "TrialNodeRecorder::sendSelf() - does not send data to a datastore\n";
    return -1;
  }
  ...
  if (theChannel.sendID(0, commitTag, idData) < 0) {
    opserr << "TrialNodeRecorder::sendSelf() - failed to send idData\n";
    return -1;
  }

  if (theDofs != 0) 
    if (theChannel.sendID(0, commitTag, *theDofs) < 0) {
      opserr << "TrialNodeRecorder::sendSelf() - failed to send dof id's\n";
      return -1;
    }

  if (theNodalTags != 0)
    if (theChannel.sendID(0, commitTag, *theNodalTags) < 0) {
      opserr << "TrialNodeRecorder::sendSelf() - failed to send nodal tags\n";
      return -1;
    }
  ...
  if (theChannel.sendVector(0, commitTag, data) < 0) {
    opserr << "TrialNodeRecorder::sendSelf() - failed to send data\n";
    return -1;
  }

  if (theOutputHandler->sendSelf(commitTag, theChannel) < 0) {
    opserr << "TrialNodeRecorder::sendSelf() - failed to send the DataOutputHandler\n";
    return -1;
  }
  ...
  if (theTimeSeries != 0) {
    ...
    if (theChannel.sendID(0, commitTag, timeSeriesTags) < 0) {
      opserr << "EnvelopeTrialNodeRecorder::sendSelf() - failed to send time series tags\n";
      return -1;
    }    
    for (int i=0; i<numDOF; i++) {
      if (theTimeSeries[i] != 0) {	
	if (theTimeSeries[i]->sendSelf(commitTag, theChannel) < 0) {
	  opserr << "EnvelopeTrialNodeRecorder::sendSelf() - time series failed in send\n";
	  return -1;
...
int 
TrialNodeRecorder::recvSelf(int commitTag, Channel &theChannel, 
		       FEM_ObjectBroker &theBroker)
{
  ...
  if (theChannel.recvID(0, commitTag, idData) < 0) {
    opserr << "TrialNodeRecorder::recvSelf() - failed to send idData\n";
    return -1;
  }
  ...
  if (theDofs == 0 || theDofs->Size() != numDOFs) {
    ...
    if (numDOFs != 0) {
      ...
      if (theDofs == 0 || theDofs->Size() != numDOFs) {
	opserr << "TrialNodeRecorder::recvSelf() - out of memory\n";
	return -1;
      }	
    }
  }
  if (theDofs != 0)
    if (theChannel.recvID(0, commitTag, *theDofs) < 0) {
      opserr << "TrialNodeRecorder::recvSelf() - failed to recv dof data\n";
      return -1;
    } 
  ...
  if (theChannel.recvVector(0, commitTag, data) < 0) {
    opserr << "TrialNodeRecorder::sendSelf() - failed to receive data\n";
    return -1;
  }
  deltaT = data(0);
  ...
  if (theOutputHandler == 0) {
    opserr << "TrialNodeRecorder::sendSelf() - failed to get a data output handler\n";
    return -1;
  }
  ...
  if (idData[7] == 1) {
    ...
    if (theChannel.recvID(0, commitTag, timeSeriesTags) < 0) {
      opserr << "EnvelopeTrialNodeRecorder::recvSelf() - failed to recv time series tags\n";
      return -1;
    }    
    for (int i=0; i<numDOFs; i++) {
	...
	if (theTimeSeries[i]->recvSelf(commitTag, theChannel, theBroker) < 0) {
	  opserr << "EnvelopeTrialNodeRecorder::recvSelf() - time series failed in recv\n";
	  return -1;
	} 
	  }
	}
...
int
TrialNodeRecorder::domainChanged(void)
{
  return 0;
}
...
int
TrialNodeRecorder::initialize(void)
{
  if (theDofs == 0 || theDomain == 0) {
    opserr << "TrialNodeRecorder::initialize() - either nodes, dofs or domain has not been set\n";
    return -1;
  }
  ...
  if (theNodalTags != 0) {
    ...
    if (theNodes == 0) {
      opserr << "TrialNodeRecorder::domainChanged - out of memory\n";
      return -1;
    }
...
//added by SAJalali
double TrialNodeRecorder::getRecordedValue(int clmnId, int rowOffset, bool reset)
{
	double res = 0;
