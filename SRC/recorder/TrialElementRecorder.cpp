...
#include <TrialElementRecorder.h>
#include <Domain.h>
#include <Element.h>
...
OPS_TrialElementRecorder()
{
    ...
    if (domain == 0)
        return 0;
    TrialElementRecorder* recorder = new TrialElementRecorder(&elements,
...
TrialElementRecorder::TrialElementRecorder()
:Recorder(RECORDER_TAGS_TrialElementRecorder),
...
TrialElementRecorder::TrialElementRecorder(const ID *ele,
				 const char **argv, 
				 int argc,
				 bool echoTime, 
				 Domain &theDom, 
 ...
 initializationDone(false), responseArgs(0), numArgs(0), addColumnInfo(0)
{
  ...
  if (ele != 0) {
    ...
    if (eleID == 0 || eleID->Size() != numEle)
      opserr << "TrialElementRecorder::TrialElementRecorder() - out of memory\n";
  } 
...
int 
TrialElementRecorder::record(int commitTag, double timeStamp)
{
  // 
  ...
  if (initializationDone == false) {
    if (this->initialize() != 0) {
      opserr << "TrialElementRecorder::record() - failed to initialize\n";
      return -1;
    }
  }
...
int
TrialElementRecorder::restart(void)
{
  if (data != 0)
    data->Zero();
  return 0;
...
int 
TrialElementRecorder::setDomain(Domain &theDom)
{
  theDomain = &theDom;
  return 0;
}
...
int
TrialElementRecorder::sendSelf(int commitTag, Channel &theChannel)
{
  addColumnInfo = 1;

  if (theChannel.isDatastore() == 1) {
    opserr << "TrialElementRecorder::sendSelf() - does not send data to a datastore\n";
    return -1;
  }
  ...
  if (theChannel.sendID(0, commitTag, idData) < 0) {
    opserr << "TrialElementRecorder::sendSelf() - failed to send idData\n";
    return -1;
  }
  ...
  if (theChannel.sendVector(0, commitTag, dData) < 0) {
    opserr << "TrialElementRecorder::sendSelf() - failed to send dData\n";
    return -1;
  }
  ...
  if (eleID != 0)
    if (theChannel.sendID(0, commitTag, *eleID) < 0) {
      opserr << "TrialElementRecorder::sendSelf() - failed to send eleID\n";
      return -1;
    }
  ...
  if (dof != 0)
    if (theChannel.sendID(0, commitTag, *dof) < 0) {
      opserr << "TrialElementRecorder::sendSelf() - failed to send dof\n";
      return -1;
    }
  ...
  if (msgLength ==  0) {
    opserr << "TrialElementRecorder::sendSelf() - no data to send!!\n";
    return -1;
  }

  char *allResponseArgs = new char[msgLength];
  if (allResponseArgs == 0) {
    opserr << "TrialElementRecorder::sendSelf() - out of memory\n";
    return -1;
  }
  ...
  if (theChannel.sendMsg(0, commitTag, theMessage) < 0) {
    opserr << "TrialElementRecorder::sendSelf() - failed to send message\n";
    return -1;
  }
  ...
  if (theOutputHandler == 0 || theOutputHandler->sendSelf(commitTag, theChannel) < 0) {
    opserr << "TrialElementRecorder::sendSelf() - failed to send the DataOutputHandler\n";
    return -1;
  }
...
int 
TrialElementRecorder::recvSelf(int commitTag, Channel &theChannel, 
		 FEM_ObjectBroker &theBroker)
{
  addColumnInfo = 1;

  if (theChannel.isDatastore() == 1) {
    opserr << "TrialElementRecorder::recvSelf() - does not recv data to a datastore\n";
    return -1;
  }
  ...
  if (theChannel.recvID(0, commitTag, idData) < 0) {
    opserr << "TrialElementRecorder::recvSelf() - failed to recv idData\n";
    return -1;
  }
  ...
  if (eleSize != 0) {
    ...
    if (eleID == 0) {
      opserr << "TrialElementRecorder::recvSelf() - failed to create eleID\n";
      return -1;
    }
    if (theChannel.recvID(0, commitTag, *eleID) < 0) {
      opserr << "TrialElementRecorder::recvSelf() - failed to recv eleOD\n";
      return -1;
    }
  }
  ...
  if (numDOF != 0) {
    dof = new ID(numDOF);
    if (dof == 0) {
      opserr << "TrialElementRecorder::recvSelf() - failed to create dof\n";
      return -1;
    }
    if (theChannel.recvID(0, commitTag, *dof) < 0) {
      opserr << "TrialElementRecorder::recvSelf() - failed to recv dof\n";
      return -1;
    }
  }
  ...
  if (msgLength == 0) {
    opserr << "TrialElementRecorder::recvSelf() - 0 sized string for responses\n";
    return -1;
  }

  char *allResponseArgs = new char[msgLength];
  if (allResponseArgs == 0) {
    opserr << "TrialElementRecorder::recvSelf() - out of memory\n";
    return -1;
  }

  Message theMessage(allResponseArgs, msgLength);
  if (theChannel.recvMsg(0, commitTag, theMessage) < 0) {
    opserr << "TrialElementRecorder::recvSelf() - failed to recv message\n";
    return -1;
  }
...
int 
TrialElementRecorder::initialize(void)
{
  if (theDomain == 0)
    return 0;
  ...
  if (eleID != 0) {
    ...
    if (theResponses == 0) {
      opserr << "TrialElementRecorder::initialize() - out of memory\n";
      return -1;
    }
...
//by SAJalali
double TrialElementRecorder::getRecordedValue(int clmnId, int rowOffset, bool reset)
{
	double res = 0;
