	...
	TCL_Char** argv, Domain& theDomain, Recorder** theRecorder)
{
	...
	// a recorder for the graphical display of the domain
	...
	else if ((strcmp(argv[1], "TrialElement") == 0)) {
		...
		(*theRecorder) = new TrialElementRecorder(eleIDs,
			data,
			argc - eleData,
			echoTime,
			theDomain,
	...
	else if ((strcmp(argv[1], "TrialNode") == 0)) {
		...
		(*theRecorder) = new TrialNodeRecorder(theDofs,
			theNodes,
			sensitivity,
			responseID,
			theDomain,
