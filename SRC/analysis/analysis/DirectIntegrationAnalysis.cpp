...
int
DirectIntegrationAnalysis::tryOneStep(int numSteps, double dT)
{
	int result = 0;
	Domain* the_Domain = this->getDomainPtr();
	...
	the_Domain->calculateNodalReactions(0);

	return result;
}

int
DirectIntegrationAnalysis::commitOneStep(void)
{
	int result = 0;
	Domain* the_Domain = this->getDomainPtr();
...
int
DirectIntegrationAnalysis::revertToLastCommit(void)
{
	Domain* the_Domain = this->getDomainPtr();
