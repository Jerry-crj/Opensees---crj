...
int
StaticAnalysis::commitOneStep(void)
{
	int result;
	Domain* the_Domain = this->getDomainPtr();
...
int
StaticAnalysis::revertToLastCommit(void)
{
	Domain* the_Domain = this->getDomainPtr();
...
int
StaticAnalysis::runOneStepDebug()
{
	Domain* the_Domain = this->getDomainPtr();
	Recorder** theRecorders = the_Domain->getRecorders();
	int numRecorders = the_Domain->getNumRecorders();

	for (int i = 0; i < numRecorders; i++)
	{
		const char* type = theRecorders[i]->getRecorderType();
		if (strcmp(type, "TrialNodeRecorder") == 0 || strcmp(type, "TrialElementRecorder") == 0)
		{
			theRecorders[i]->record(0, 0);
		}
	}

	int result = theAnalysisModel->analysisStep();
	...
	theTest->setEquiSolnAlgo(*theAlgorithm);
	if (theTest->start() < 0) {
	...
	do {
		if (theIntegrator->formTangent(CURRENT_TANGENT, 0.0, 1.0) < 0) {
			opserr << "WARNING NewtonRaphson::solveCurrentStep() -";
		...
		result = theTest->test();
		numIterations++;
		theAlgorithm->record(numIterations);
		...
		for (int i = 0; i < numRecorders; i++)
		{
			...
			if (strcmp(type, "TrialNodeRecorder") == 0 || strcmp(type, "TrialElementRecorder") == 0)
			{
				theRecorders[i]->record(0, numIterations);
			}
		}
	...
	the_Domain->setNumRecordersZero();
