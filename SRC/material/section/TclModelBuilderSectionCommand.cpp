...
//#include <FiberSectionTimoFourOrder.h>
//#include <FiberSectionTimoFourOrderCurveSectio.h>
...
int
TclCommand_addTimoFiberSection(ClientData clientData, Tcl_Interp* interp, int argc,
	TCL_Char** argv, TclModelBuilder* theTclModelBuilder);

int
buildTimoSection(Tcl_Interp* interp, TclModelBuilder* theTclModelBuilder,
	int secTag, int argc, TCL_Char** argv);
	...
	TCL_Char** argv, TclModelBuilder* theTclBuilder)
{
	...
	else if (strcmp(argv[1], "FiberSectionTimoConstShear") == 0 ||
		strcmp(argv[1], "FiberSectionTimoQualShear") == 0)
		return TclCommand_addTimoFiberSection(clientData, interp, argc, argv,
			theTclBuilder);
	...
	TCL_Char** argv, TclModelBuilder* theTclModelBuilder)
{
	...
	int brace = 3; // Start of recursive parse
	if (strcmp(argv[1], "FiberSectionTimoQualShear") == 0)
		brace = brace + 2;
	...
	if (buildTimoSection(interp, theTclModelBuilder, secTag, argc, argv) != TCL_OK) {
		opserr << "WARNING - error constructing the section\n";
		return TCL_ERROR;
	}
	...
	int secTag, int argc, TCL_Char** argv)
{
	...
	if (sectionRepres->getType() == SEC_TAG_FiberSection)
	{
		...
		if (strcmp(argv[1], "FiberSectionTimoConstShear") == 0)
			section = new FiberSectionTimoConstShear(secTag, numFibers, fiber);
		else if (strcmp(argv[1], "FiberSectionTimoQualShear") == 0)
		{
			section = new FiberSectionTimoQualShear(secTag, numFibers, fiber);
			FiberSectionTimoQualShear* theSec = (FiberSectionTimoQualShear*)section;

			double b, h;

			if (Tcl_GetDouble(interp, argv[3], &h) != 0 ||
				Tcl_GetDouble(interp, argv[4], &b) != 0)
			{
				opserr << "WARNING get section size failed \n";
				return TCL_ERROR;
			}
			theSec->setSectionSize(h, b);
		}
		// Delete fibers
		for (i = 0; i < numFibers; i++)
			delete fiber[i];
