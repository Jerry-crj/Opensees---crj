...
ovalConcrete::setTrialStrain(const Vector& trialStrain)
{
	...
	if (sig(0) > 0)
	{
		if (sig(1) == 0 && (dmgtsp == 1.0) || (dmgtsn == 1.0))
		{
			sig.Zero();
			e.Zero();
			e(0, 0) = 1.0e-10;
...
{
	if (et(0) >= epsresn  && et(1) >= 0)
	{
		if (epsP(0) < epsresn && epsP(1) > 0)
		{
			st(0) = ktP * (et(0) - epsresn);
			st(1) = sigP(1) + ktspP * (et(1) - epsP(1));
			tgt(0, 0) = ktP;
			tgt(1, 1) = ktspP;
		...
		else if (epsP(0) < epsresn && epsP(1) < 0)
		{
			st(0) = ktP * (et(0) - epsresn);
			st(1) = ktspP * et(1);
			tgt(0, 0) = ktP;
		...
		else
		{
			...
			tgt(0, 0) = ktP;
			tgt(1, 1) = ktspP;
			double sigMax, tauMax, sigMin, tauMin;
			sigMax = sigP(0) + knInit * (et(0) - epsP(0));
			tauMax = sigP(1) + ksInit * (et(1) - epsP(1));
			sigMin = ktP * (et(0) - epsresn);
			tauMin = ktspP * et(1);
	...
	else if (et(0) <= epsresn && et(1) >= 0)
	{
		if (epsP(0) > epsresn && epsP(1) > 0)
		{
			st(0) = kcP * (et(0) - epsresn);
			st(1) = sigP(1) + kcspP * (et(1) - et(1));
			tgt(0, 0) = kcP;
			tgt(1, 1) = kcspP;
			double tauMax, tauMin;
			tauMax = sigP(1) + ksInit * (et(1) - epsP(1));
	...
	if (st(0) * (et(0) - epsresn) < 0)
	{
		st(0) = 0.0;
		tgt(0, 0) = 1.0e-10;
	}

	//if (et(1) > epsresn && et(1) < epsressp)
	//{
	//	st(1) = 0.0;
	//	tgt(1, 1) = 1.e-10;
	//}
