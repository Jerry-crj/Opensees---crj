...
int OpenSeesAppInit(Tcl_Interp* interp) {
	...
	//Tcl_CreateCommand(interp, "runOneStep", &runOneStep,
	//	(ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
	Tcl_CreateCommand(interp, "tryOneStep", &tryOneStep,
		(ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
	Tcl_CreateCommand(interp, "commitOneStep", &commitOneStep,
		(ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);///**/
	Tcl_CreateCommand(interp, "revertToLastCommit", &revertToLastCommit,
		(ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);///**/

	//Tcl_CreateCommand(interp, "getMaterialTrialStress", &getMaterialTrialStress,
	//    (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);///**/
	//Tcl_CreateCommand(interp, "getMaterialStiff", &getMaterialStiff,
	//    (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);///**/
	//Tcl_CreateCommand(interp, "runOneStepDebug", &runOneStepDebug,
	//    (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);///**/
...
int
tryOneStep(ClientData clientData, Tcl_Interp* interp, int argc, TCL_Char** argv)
{
	int result = 0;

	if (theStaticAnalysis != 0) {
		result = theStaticAnalysis->tryOneStep();
	}
	else if (theTransientAnalysis != 0) {
		if (argc < 3) {
			opserr << "WARNING transient analysis: analysis numIncr? deltaT?\n";
		...
		if (argc == 6) {
			...
			result = theTransientAnalysis->tryOneStep(numIncr, dT);
		}
	}
	else {
		opserr << "WARNING No Analysis type has been specified \n";
...
// add for Client-Server

int
commitOneStep(ClientData clientData, Tcl_Interp* interp, int argc, TCL_Char** argv)
{
	int result = 0;

	if (theStaticAnalysis != 0) {
		result = theStaticAnalysis->commitOneStep();
	}
	if (theTransientAnalysis != 0) {
		result = theTransientAnalysis->commitOneStep();
	}
	else {
		opserr << "WARNING No Analysis type has been specified \n";
		return TCL_ERROR;
...
int
revertToLastCommit(ClientData clientData, Tcl_Interp* interp, int argc, TCL_Char** argv)
{

	int result = 0;

	if (theStaticAnalysis != 0) {
		result = theStaticAnalysis->revertToLastCommit();
	}
	if (theTransientAnalysis != 0) {
		result = theTransientAnalysis->revertToLastCommit();
	}
	else {
		opserr << "WARNING No Analysis type has been specified \n";
		return TCL_ERROR;
...
int
runOneStepDebug(ClientData clientData, Tcl_Interp* interp, int argc, TCL_Char** argv)
{
	int result = 0;

	if (theStaticAnalysis != 0) {
		result = theStaticAnalysis->runOneStepDebug();
	}
	//else if (theTransientAnalysis != 0) {
	//	if (theVariableTimeStepTransientAnalysis != 0) 
 //       {
	//		opserr << "commitOneStep for static analysis is not done yet... " << endln;
	//		exit(-1);
	//	}
	//	else
 //       {
...
//#include <..\..\..\SRC\element\XMUelements\TimoshenkoModified.h>
//#include <SectionForceDeformation.h>
//#include <..\..\..\SRC\material\FiberSectionTimo.h>
//#include <NDMaterial.h>
//#include <..\..\..\SRC\material\ovalConcrete.h>
////#include <SingleDomEleIter.h>
//
//int
//getMaterialTrialStress(ClientData clientData, Tcl_Interp* interp, int argc, TCL_Char** argv)
//{
//	if (&theDomain != 0 && (theStaticAnalysis != 0 || theTransientAnalysis != 0)) {
//        if (argc == 13 && strcmp(argv[1], "element") == 0 && strcmp(argv[3], "section") == 0 
//            && strcmp(argv[4], "fiber") == 0 && strcmp(argv[7], "strainP") == 0 
//            && strcmp(argv[10], "strain") == 0)
//        {
//			int eleTag;
//			double yLoc, zLoc;
//            if (Tcl_GetInt(interp, argv[2], &eleTag) != TCL_OK)
//            {
//                opserr << "WARNING element tag failed to get integer: " << argv[2] << endln;
//                return TCL_ERROR;
//            }
//            if (Tcl_GetDouble(interp, argv[5], &yLoc) != TCL_OK || Tcl_GetDouble(interp, argv[6], &zLoc) != TCL_OK)
//            {
//                opserr << "WARNING fiber location failed to get double: " << argv[5] << endln;
//                return TCL_ERROR;
//            }
//            Element* theEle = theDomain.getElement(eleTag);
//            const char* type = theEle->getClassType();
//            if (strcmp(type, "TimoshenkoModified") != 0)
//            {
//                opserr << "WARNING the element type should be TimoshenkoModified";
//                return TCL_ERROR;
//            }
//            TimoshenkoModified* spcfEle = (TimoshenkoModified*)theEle;
//            SectionForceDeformation* theSec= spcfEle->getSectionPtr();
//            type = theSec->getClassType();
//            if (strcmp(type, "FiberSectionTimo") != 0)
//            {
//                opserr << "WARNING the element type should be TimoshenkoModified";
//                return TCL_ERROR;
//            }
//            FiberSectionTimo* spcfSec = (FiberSectionTimo *) theSec;
//            NDMaterial* objMat = spcfSec->getMatPtr(yLoc, zLoc);
//            double strainPValue[2], strainValue[2];
//            if (Tcl_GetDouble(interp, argv[8], strainPValue) != TCL_OK ||
//                Tcl_GetDouble(interp, argv[9], strainPValue + 1) != TCL_OK ||
//                Tcl_GetDouble(interp, argv[11], strainValue) != TCL_OK ||
//                Tcl_GetDouble(interp, argv[12], strainValue + 1) != TCL_OK)
//            {
//                opserr << "input two strain value " << endln;
//                return TCL_ERROR;
//            }
//            Vector strain(strainValue, 2), strainP(strainPValue, 2);
//            Vector stress(2), deltaStrain(2), trialStrain(2);
//            deltaStrain = strain - strainP;
//
//            ofstream file;
//            file.open("trialStress.txt");
//
//            for (int i = 0; i <= 10; i++)
//            {
//                trialStrain = strainP + i / 10. * deltaStrain;
//                objMat->setTrialStrain(trialStrain);
//                stress = objMat->getStress();
//                file << i << "     " << stress(0) << "     " << stress(1) << "     " << trialStrain(0) << "     " << trialStrain(1) << endln;
//            }
//            file.close();
//        }
//        else
//        {
//            opserr << "the type should be element? eleTag? section? fiber? fiberLoc? " << endln;
//        }
//    }
//    else {
//        opserr << "WARNING No Analysis type has been specified \n";
...
//
//int
//getMaterialStiff(ClientData clientData, Tcl_Interp* interp, int argc, TCL_Char** argv)
//{
//    if (&theDomain != 0 && (theStaticAnalysis != 0 || theTransientAnalysis != 0)) {
//        if (argc == 10 && strcmp(argv[1], "element") == 0 && strcmp(argv[3], "section") == 0
//            && strcmp(argv[4], "fiber") == 0 && strcmp(argv[7], "strain") == 0)
//        {
//            int eleTag;
//            double yLoc, zLoc;
//            if (Tcl_GetInt(interp, argv[2], &eleTag) != TCL_OK)
...
//            }
//            FiberSectionTimo* spcfSec = (FiberSectionTimo*)theSec;
//            NDMaterial* theMat = spcfSec->getMatPtr(yLoc, zLoc);
//            type = theMat->getType();
//            if (strcmp(type, "ovalConcrete") != 0)
//            {
//                opserr << "WARNING the element type should be ovalConcrete";
//                return TCL_ERROR;
//            }
//            ovalConcrete* spcfMat = (ovalConcrete*)theMat;
//
//            double strainValue[2];
//            if (Tcl_GetDouble(interp, argv[8], strainValue) != TCL_OK ||
//                Tcl_GetDouble(interp, argv[9], strainValue + 1) != TCL_OK)
//            {
//                opserr << "input two strain value " << endln;
//                return TCL_ERROR;
//            }
//            Vector strain(strainValue, 2);
//
//			double pv[28];
//			pv[0] = 1.00e-05;
//			pv[1] = 1.00e-06;
...
//            
//            ofstream file;
//            file.open("trialTangent.txt");
//
//
//            for (int i = 0; i < 28; i++)
//            {
//                spcfMat->setPerturbValue(pv[i]);
//                spcfMat->setTrialStrain(strain);
//                Matrix stiff = spcfMat->getTangent();
//                file << pv[i] << "     " << stiff(0, 0) << "     " << stiff(1, 0) << "     " << stiff(0, 1) << "     " << stiff(1, 1) << endln;
//            }
//            file.close();
//        }
//        else
