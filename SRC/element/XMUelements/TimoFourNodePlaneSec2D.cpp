...
#include <TimoFourNodePlaneSec2D.h>
#include <Node.h>
...
Matrix TimoFourNodePlaneSec2D::B(3, 12);
Matrix TimoFourNodePlaneSec2D::Btran(12, 3);
Matrix TimoFourNodePlaneSec2D::K(12, 12);
Vector TimoFourNodePlaneSec2D::P(12);
//double TimoFourNodePlaneSec2D::workArea[100];

void *
OPS_TimoFourNodePlaneSec2D(void)
{
	...
	if (OPS_GetIntInput(&numData, iData) != 0) {
		opserr << "TimoFourNodePlaneSec2D::WARNING: invalid integer inputs \n";
		return 0;
	}
	...
	theEle = new TimoFourNodePlaneSec2D(eleTag, iNode, jNode, kNode, lNode, numSec, secs, *beamIntegr);
...
TimoFourNodePlaneSec2D::TimoFourNodePlaneSec2D(int tag, int nd1, int nd2, int nd3, int nd4,
	int numSec, SectionForceDeformation** s,
	BeamIntegration& bi)
	:Element(tag, ELE_TAG_TimoFourNodePlaneSec2D),
	...
	connectedExternalNodes(4), T(12, 12), Ttran(12, 12)
{
	...
	if (theSections == 0) {
		opserr << "TimoFourNodePlaneSec2D::TimoFourNodePlaneSec2D - failed to allocate section model pointer\n";
		exit(-1);
	}
	...
	for (int i = 0; i < numSections; i++) {
		...
		// Check allocation
		if (theSections[i] == 0) {
			opserr << "TimoFourNodePlaneSec2D::TimoFourNodePlaneSec2D -- failed to get a copy of section model\n";
			exit(-1);
		}
	}
	...
	if (beamInt == 0) {
		opserr << "TimoFourNodePlaneSec2D::TimoFourNodePlaneSec2D - failed to copy beam integration\n";
		exit(-1);
	}
...
TimoFourNodePlaneSec2D::TimoFourNodePlaneSec2D()
	:Element(0, ELE_TAG_TimoFourNodePlaneSec2D),
...
int
TimoFourNodePlaneSec2D::getNumExternalNodes() const
{
	return 4;
}

const ID&
TimoFourNodePlaneSec2D::getExternalNodes()
{
	return connectedExternalNodes;
}

Node**
TimoFourNodePlaneSec2D::getNodePtrs()
{
	return theNodes;
}

int
TimoFourNodePlaneSec2D::getNumDOF()
{
	return 12;
}

void
TimoFourNodePlaneSec2D::setDomain(Domain* theDomain)
{
	...
	if (theNodes[0] == 0 || theNodes[1] == 0 || theNodes[2] == 0 || theNodes[3] == 0) {
		opserr << "WARNING TimoFourNodePlaneSec2D (tag: %d), node not found in domain" << this->getTag() << endln;;
		return;
	}
	...
	if (L == 0.0) {
		opserr << "WARNING TimoFourNodePlaneSec2D (tag: %d), the length should not be 0" << this->getTag() << endln;;
		return;
	}
...
int
TimoFourNodePlaneSec2D::commitState()
{
	int retVal = 0;

	// call element commitState to do any base class stuff
	if ((retVal = this->Element::commitState()) != 0) {
		opserr << "TimoFourNodePlaneSec2D::commitState () - failed in base class";
	}
...
int
TimoFourNodePlaneSec2D::revertToLastCommit()
{
	int retVal = 0;
...
int
TimoFourNodePlaneSec2D::revertToStart()
{
	int retVal = 0;
...
int
TimoFourNodePlaneSec2D::update(void)
{
	int err = 0;
	...
	if (err != 0) {
		opserr << "TimoFourNodePlaneSec2D::update() - failed setTrialSectionDeformations()\n";
		return err;
	}
...
const Matrix&
TimoFourNodePlaneSec2D::getTangentStiff()
{
	double xi[maxNumSections];
	beamInt->getSectionLocations(numSections, L, xi);
...
const Vector&
TimoFourNodePlaneSec2D::getResistingForce()
{
	double xi[maxNumSections];
	beamInt->getSectionLocations(numSections, L, xi);
	double wt[maxNumSections];
...
void
TimoFourNodePlaneSec2D::calculateB(double xi)
{
	B.Zero();
	...
	B(2, 1) = -(11 - 36 * xi + 27 * xi * xi) / (2 * L);
	B(2, 4) = 9 * (2 - 10 * xi + 9 * xi * xi) / (2 * L);
	B(2, 7) = -9 * (1 - 8 * xi + 9 * xi * xi) / (2 * L);
	B(2, 10) = (2 - 18 * xi + 27 * xi * xi) / (2 * L);
	B(2, 2) = -(1 - 3 * xi) * (2 - 3 * xi) * (1 - xi) /2;
	B(2, 5) = -9 * (2 - 3 * xi) * (1 - xi) * xi / 2;
	B(2, 8) = 9 * (1 - 3 * xi) * (1 - xi) * xi / 2;
	B(2, 11) = -(1 - 3 * xi) * (2 - 3 * xi) * xi / 2;
...
const Matrix&
TimoFourNodePlaneSec2D::getInitialStiff()
{
	return K;
}

const Matrix&
TimoFourNodePlaneSec2D::getMass()
{
	return K;
}

void
TimoFourNodePlaneSec2D::zeroLoad(void)
{

}

int
TimoFourNodePlaneSec2D::addLoad(ElementalLoad* theLoad, double loadFactor)
{
	return 0;
}

int
TimoFourNodePlaneSec2D::addInertiaLoadToUnbalance(const Vector& accel)
{
	return 0;
}

const Vector&
TimoFourNodePlaneSec2D::getResistingForceIncInertia()
{
...
int
TimoFourNodePlaneSec2D::sendSelf(int commitTag, Channel& theChannel)
{
	return 0;
}

int
TimoFourNodePlaneSec2D::recvSelf(int commitTag, Channel& theChannel,
...
void
TimoFourNodePlaneSec2D::Print(OPS_Stream& s, int flag)
{
	s << "\nTimoFourNodePlaneSec2D, element id:  " << this->getTag() << endln;
...
int
TimoFourNodePlaneSec2D::displaySelf(Renderer& theViewer, int displayMode, float fact)
{
...
Response*
TimoFourNodePlaneSec2D::setResponse(const char** argv, int argc,
	OPS_Stream& output)
{
	Response* theResponse = 0;

	output.tag("ElementOutput");
	output.attr("eleType", "TimoFourNodePlaneSec2D");
...
int
TimoFourNodePlaneSec2D::getResponse(int responseID, Information& eleInfo)
{
	double V;
